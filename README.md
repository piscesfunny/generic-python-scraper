# Generic Python Scraper
Python Web Scraping tool to extract data from various websites

## Get Categories
    python main.py alibaba get_category
    
## Get URLS List for specific category
    python main.py alibaba scraping Tractors list
    
## Get items for specific category    
    python main.py alibaba scraping Tractors item
    
## Download media files:
    python main.py alibaba download Tractors

## Specific category

### Get item List
    python main.py alibaba scraping circuit_breakers list unknown https://www.alibaba.com/catalog/circuit-breakers_cid141914 100 1
### Get items
    python main.py alibaba scraping transformer item unknown


## Run by spider
### online_epocrates
    $ python main.py online_epocrates scraping drugs list
    $ python main.py online_epocrates scraping drugs item